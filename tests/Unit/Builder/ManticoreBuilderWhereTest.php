...
it('whereNot with gt operator produces lte', function () {
    $sql = builderSql(makeBuilder()->whereNot('score', '>', 100)->limit(5));
    expect($sql)->toContain('`score` <= 100');
});

it('whereNot with gte operator produces lt', function () {
    $sql = builderSql(makeBuilder()->whereNot('score', '>=', 100)->limit(5));
    expect($sql)->toContain('`score` < 100');
});

it('whereNot with lt operator produces gte', function () {
    $sql = builderSql(makeBuilder()->whereNot('score', '<', 50)->limit(5));
    expect($sql)->toContain('`score` >= 50');
});

it('whereNot with lte operator produces gt', function () {
    $sql = builderSql(makeBuilder()->whereNot('score', '<=', 50)->limit(5));
    expect($sql)->toContain('`score` > 50');
});
